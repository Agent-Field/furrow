# furrow — Document 3: Parallel Universes — Many Agents, One Repo
...
> **Worktrees were built for one human switching branches. furrow is built for many agents sharing one repo — with instant forks, live conflict radar, and merges that are already done when the agents finish.**
...
### A1. Same path, different universes (`furrow exec`)

```text
furrow exec --fork a -- claude -p "fix the auth bug"
furrow exec -n 3 -- <cmd>          # three universes, forks created implicitly
```
...
- macOS: no mount namespaces — `exec` falls back to sibling directories with the fork path exported as `$FURROW_WORKDIR` and injected as the process CWD. Honest per-platform behavior, disclosed by `furrow exec --plan` (consistent with Doc 1 Invariant 11).
...
- Linux: each `exec` universe gets a **network namespace**; every agent binds `:3000` privately. furrow maps each universe's ports to distinct host ports (veth + forward) and reports the table.
...
- Surfacing: **not a new command.** Port mappings appear in `furrow forks` output and in the MCP `fork_info` result.
...
- Human surfacing: a `conflicts` column in `furrow forks`; nothing else.
- Agent surfacing: `fork_conflict` events on the `furrow events --follow` NDJSON stream (path, forks involved, claim state — mirrored to MCP notifications per §6.3) and a `coord`-class advisory file — so agents *route around* collisions instead of creating them. Radar events suggest claims; claims remain advisory (Doc 2 §4, R7).
...
- At any moment, `furrow status` answers: `merge: clean ✓ check: passing ✓ (as of seal 9f3c, 40s ago)` — or names the conflicting pair and file.
- When agents finish, the final `furrow merge` is a promotion of already-computed state: merge latency ≈ 0.
...
- Resource honesty: preview runs at background priority, pauses on battery/thermal (Doc 1 §10.3 signals), and its check command execution is budgeted (`.furrowpolicy: preview.check = on-idle | on-seal | off`). Default: merge computation always (cheap), check execution on-idle.
- **Not a command.** Preview state is a field in `status`/`forks` and an MCP resource. `furrow merge --preview` prints the full detail on demand.
...
- Requires CAP_BPF or equivalent; auto-enabled when available, visible in `furrow status`, never required.
...
- **D9. One fork table, any location.** A universe on a Hetzner box or an AgentField subharness node appears in `furrow forks` exactly like a local one — same diff, same radar, same merge preview, same claims. "Distributed" is a column, not a mode. Requires: coord-class eager replication across machines, seal streaming (target: remote activity visible locally within seconds), signed runner seals (Doc 2 §2.3).
- **D10. `furrow exec -n 5 --cloud -- <cmd>`.** The capstone: five universes — three local, two remote — side-by-side output, radar between all of them, merge preview at the bottom, gate-verified winner at the end. This is the demo that defines the category, and it must not be attempted until A–C make the local version boring.
...
- Global `--json` on every command; `FURROW_AGENT=1` (or non-TTY detection) disables prompts, confirmations-by-question, colors, and relative-time parsing.
...
- **Events are CLI-native:** `furrow events --follow` streams NDJSON (radar conflicts, preview transitions, seal/merge completions, remote-fork activity later). Agents background or poll it; hooks and MCP notifications are mirrors of this one stream, never separate sources.
...
Interactive wizards, config subcommand trees (`.furrowpolicy` is the config surface), alias systems, plugins-in-core, per-framework helpers (fork hooks cover it), and any command whose output a column in `forks`/`status` could carry.

---

## 7. The One UI: Mission Control

**Yes to exactly one UI** — a local, daemon-served web page (`furrow ui`, localhost) — and no others. Resolves Doc 1 open question 4 in favor of web over TUI.
...
|---|---|---|---|
| R1 | Unprivileged userns unavailable (hardened kernels, some distros) | Detect; `exec` falls back to sibling-dir mode with `$FURROW_WORKDIR`; disclosed via `exec --plan` | Same-path magic is Linux-mainstream, not universal |
...
3. Mission Control packaging: bundled in the single binary (adds size) vs separate `furrow-ui` artifact fetched on first `furrow ui`.
4. Per-tool-call seal labels: adopt a generic hook naming convention now so any harness's tool names render in the timeline without furrow knowing the harness (candidate: `FURROW_LABEL` env read at forced-boundary seals).
